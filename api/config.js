...
const SUBS_KEY = "dash_monitor_tag_submissions";
  ...
  if (req.method === "POST") {
    ...
    if (body.watchedAddresses !== undefined) {
      ...
      for (const w of body.watchedAddresses) {
        ...
        if (!/^[X7][1-9A-HJ-NP-Za-km-z]{33}$/.test(addr)) {
          return res
            .status(400)
            .json({ error: `invalid address: "${addr.slice(0, 50)}" — must be a valid Dash address (X... or 7...)` });
        }
        const label =
